...
def test_major_below_eighty_is_rejected_for_four_year():
    ...
    issues, _ = validate_stage("ugc_curriculum", data, ctx("UG - 4 Year (Honours with Research)"))
...
def test_value_added_courses_respect_the_six_to_eight_range():
    issues, _ = validate_stage("ugc_curriculum", _matrix(vac=9), ctx("UG - 3 Year", 6))
    assert any("the UGC range is 6 to 8" in m for m in errors(issues))

    issues, _ = validate_stage("ugc_curriculum", _matrix(vac=5), ctx("UG - 3 Year", 6))
...
def test_internship_respects_the_two_to_four_range():
    issues, _ = validate_stage("ugc_curriculum", _matrix(internship=5), ctx("UG - 3 Year", 6))
...
def test_research_does_not_apply_to_a_three_year_programme():
    issues, _ = validate_stage("ugc_curriculum", _matrix(research=12), ctx("UG - 3 Year", 6))
...
def test_four_year_needs_one_hundred_and_sixty():
    ...
    issues, _ = validate_stage("ugc_curriculum", data,
                               ctx("UG - 4 Year (Honours with Research)"))
    msgs = errors(issues)
